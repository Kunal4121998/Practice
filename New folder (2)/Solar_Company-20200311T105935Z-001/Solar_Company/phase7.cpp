...
int updateFile(int index, int schedule_id){
    ...
    bool ccheck[days][no_of_vendors+2];
    ...
    for(int i=0;i<days;i++){
        for(int j=0;j<no_of_vendors;j++){
            if(P[index].SupplierName[j].ASNDate.size() > 0){
                for(int k=0;k<P[index].SupplierName[j].ASNDate.size();k++){
                    if(dateString[i] == P[index].SupplierName[j].ASNDate[k]){
                        ccheck[i + P[index].SupplierName[j].Duration][1+j] = false;
                    }
                }
            }
        }
    ...
    for(int i=0;i<7;i++){
        ...
        if(!skip || i>0){
            ...
            if((ClosingStock+consume) < MaximumStock && (maxCapOfVendor[Flag]+lotSize[Flag]) >= consume && Flag!=-1){
                if(lotSize[Flag] > 0){
                    if(TotalPOQuantity[Flag] > lotSize[Flag]){
                        ...
                        if((ClosingStock + consume + TotalPOQuantity[Flag]) <= MaximumStockMax){
                            ...
                            if((i-P[index].SupplierName[Flag].Duration) >= 0){
                                DD = dateString[i-P[index].SupplierName[Flag].Duration];
                            }
                            else{
                                DD = "";
                            ...
                            ccheck[i][Flag+1] = false;
                            if(DD.size()!=0){
                                fin << P[index].PlantId<<","<<POID[Flag] <<","<<P[index].SupplierName[Flag].TotalQuan[0]<<","<<P[index].SupplierName[Flag].VendorName <<",1," << P[index].MaterialId <<"," <<P[index].MaterialName<<",EA,SCH_"<<schedule_id<<","<<DD <<","<< abs(table[i][1+Flag]) <<",No"<<",ASN_"<<schedule_id++<<","<<  endl;
                            }
                    ...
                    if(TotalPOQuantity[Flag] > (diff + (ddiff/10))){
                        ...
                        if(TotalPOQuantity[Flag] <= MaximumStockMax){
                            ...
                            ccheck[i][Flag+1] = false;
                            if(DD.size() != 0){
                                fin << P[index].PlantId<<","<<POID[Flag] <<","<<P[index].SupplierName[Flag].TotalQuan[0]<<","<<P[index].SupplierName[Flag].VendorName <<",1," << P[index].MaterialId <<"," <<P[index].MaterialName<<",EA,SCH_"<<schedule_id<<","<<DD <<","<< abs(table[i][1+Flag] - table[i + P[index].SupplierName[Flag].Duration][1 + Flag]) <<",No"<<",ASN_"<<schedule_id++<<","<<  endl;
            ...
            for(int j=0;j<no_of_vendors;j++){
                if(abs(table[i][1+j])>0){
                    float ll = abs(table[i][1+j]) - P[index].SupplierName[j].MaxCapacity;
                    if(ll>0){
                    ...
                    string DD = dateString[i];
                    if((i-P[index].SupplierName[j].Duration) >= 0){
                        DD = dateString[i-P[index].SupplierName[j].Duration];
                    }
                    else{
                        DD = "";
                    ...
                    if(DD.size() !=0){
                        fin << P[index].PlantId<<","<<POID[j] <<","<<P[index].SupplierName[j].TotalQuan[0]<<","<<P[index].SupplierName[j].VendorName <<",1," << P[index].MaterialId <<"," <<P[index].MaterialName<<",EA,SCH_"<<schedule_id<<","<<DD <<","<< abs(table[i][1+j]) - ll <<",No"<<",ASN_"<<schedule_id++<<","<< endl;
